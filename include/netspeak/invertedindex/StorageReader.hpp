...
private:
  ...
  inline bool ReadHead(FILE* file, uint32_t offset, Head& head, std::false_type) const {
  ...
  // Thread-safe version.
  inline bool ReadHead(FILE* file, uint32_t offset, Head& head, std::true_type) const {
    std::lock_guard<std::mutex> lock(mutex_);
    return ReadHead(file, offset, head, std::false_type());
  }

  inline std::unique_ptr<Postlist<T> > ReadPostlist(const fs::path& path, FILE* file, uint32_t offset, uint32_t begin,
  ...
  inline std::unique_ptr<Postlist<T> > ReadPostlist(const fs::path& path, FILE* file, uint32_t offset, uint32_t begin,
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    return ReadPostlist(path, file, offset, begin, length, page_size, std::false_type());
  }
...
public:
  ...
  void Open(const fs::path& directory, util::memory_type memory) {
    ...
    // open data files
    const size_t data_file_count(std::distance(fs::directory_iterator(data_dir), fs::directory_iterator()));
   ...
   */
  std::unique_ptr<Postlist<T> > ReadPostlist(const std::string& key, uint32_t begin = 0,
    ...
    if (table_ && table_->Get(key, address) && address.e1() < files_.size()) {
      ...
      postlist = ReadPostlist(path, file, offset, begin, length, page_size, IsThreadSafe());
    }
    return postlist;
  }
